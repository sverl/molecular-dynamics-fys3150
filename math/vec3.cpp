...
void vec3::print() {
  ...
  cout << "[" << components[0] << ", " << components[1] << ", " << components[2]
       << "]" << endl;
}

void vec3::print(string name) {
  // Will print matlab syntax vector with a name. Output will be like: A =
  // [2.09, 5.3, 9.1];
  cout << name << " = ";
  print();
}
...
vec3 vec3::cross(vec3 aVector) {
  return vec3(y() * aVector.z() - z() * aVector.y(),
...
std::ostream& operator<<(
    std::ostream& os,
    const vec3& myVector)  // Allows cout << myVector << endl;
{
